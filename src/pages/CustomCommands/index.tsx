...
export default function CustomCommands() {
    ...
    try {
      ...
      if (response.code === 200 && response.data) {
        ...
        const categoriesList = Object.entries(data).map(([key, value]) => {
          const categoryData = value as { name?: string; description?: string; data?: Record<string, string> };
  ...
  const saveData = async () => {
    try {
      ...
      const categoriesMap: Record<string, { name: string; description: string; data: Record<string, string> }> = {};
      
      values.categories.forEach((cat: { key: string; name: string; description: string; items: Array<{ key: string; name: string }> }) => {
                        ...
                        {(itemFields, { add: addItem, remove: removeItem }) => (
                          <div style={{ marginLeft: '16px' }}>
                            {itemFields.map(({ key: itemKey, name: itemName, ...restItemField }) => (
                              <Space key={itemKey} style={{ marginBottom: '8px' }} align="baseline">
                                <Form.Item
                                  {...restItemField}
                                  name={[itemName, 'key']}
                                ...
                                >
                                  <Input placeholder="物品Key" style={{ width: '200px' }} />
                                </Form.Item>
                                ...
                                <Form.Item
                                  ...
                                  style={{ marginBottom: 0 }}
                                >
                                  <Input placeholder="物品名称" style={{ width: '200px' }} />
                                </Form.Item>

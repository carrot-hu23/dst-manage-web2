...
export default () => {
    ...
    const fetchLevels = async () => {
        try {
            ...
            if (resp.code === 200) {
                ...
                setLevels(newLevels);
                setImageUrls(newLevels.map(l => `/api/dst/map/image?clusterName=${cluster}&levelName=${l.uuid}&t=${Date.now()}`));
    ...
    // 刷新所有图片
    const refreshImage = () => {
        setImageUrls(levels.map(l => `/api/dst/map/image?clusterName=${cluster}&levelName=${l.uuid}&t=${Date.now()}`));
    };
